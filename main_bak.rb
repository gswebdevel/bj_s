...
class Game
  ...
  def start_play_game(session)
   ...
   # while play_more != "N"
      ...
      session[:player_one].hand = Hand.new
      session[:player_dealer].hand = Hand.new

      #puts

      session[:player_one].hand.add_a_card(shuffled_deck.current_deck_shuffled.pop)
      session[:player_dealer].hand.add_a_card(shuffled_deck.current_deck_shuffled.pop)
      session[:player_one].hand.add_a_card(shuffled_deck.current_deck_shuffled.pop)
      session[:player_dealer].hand.add_a_card(shuffled_deck.current_deck_shuffled.pop)
  ...
  def calculate_winner(player, dealer)
    ...
    if dealer.hand.hand_value > 21
      puts  "Dealer busted, dealer hand of '#{dealer.hand.hand_cards.join("','")}' has a hand value of #{dealer.hand.hand_value}"
      player.current_wins += 1
    elsif player.hand.hand_value > 21
      puts "You busted, your hand of '#{player.hand.hand_cards.join("','")}' has a hand value of #{player.hand.hand_value}"
      dealer.current_wins += 1
...
post '/hit' do
  session[:player_one].hand.hit_or_stay(session[:shuffled_deck].current_deck_shuffled)
  erb :board
end

post '/stay' do
  session[:player_dealer].hand.hit_or_stay_dealer(session[:shuffled_deck].current_deck_shuffled)
...
post '/play_again' do
  current_game = Game.new(session[:name], session)
  current_game.start_play_game(session)
  erb :board
end
